...
class RegisterController extends ApiController
{
    ...
    protected function validValue($type, $value)
    {
        if (!isset($value)) {
            return $this->successResponse(true, 200);
        }

        $queryPlayer = Player::query();
        $queryUser = User::query();

        switch ($type) {
            case 'document':
                $data = explode("-", $value);
                $query = $queryPlayer->where('document_type', $data[0])->where('document_number', $data[1]);
                break;

            case 'nick':
                $query = $queryUser->where('nick', $value);
                break;

            case 'email':
                $query = $queryUser->where('email', $value);
                break;

            case 'phone':
                $query = $queryPlayer->where('phone', $value);
                break;

            default:
                throw new \InvalidArgumentException("Tipo de valor inválido");
        }

        return $this->successResponse(!$query->exists(), 200);
    }
}